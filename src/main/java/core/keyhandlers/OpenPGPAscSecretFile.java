...
public class OpenPGPAscSecretFile extends OpenPGPAscFile {
    ...
    protected byte [] addKeyData(KeyObject[] key, KeyIdentifier[] idDetails, KeyHandlerParameters[] parameters) throws KeyHandlerException {
        try{
            ...
            for (int n = 0; n < key.length; n++) {
                ...
                if (n == 0) { // this is the primary key
                    ...
                    if (key[n] instanceof OpenPGPKeyData) { // if this is an OpenPGPKeyData key then try and import the key packet.
                        ...
                        currentKeyPacket = new SecretKeyPacket(currentParam.getCreationDate(), currentParam.getPublicKeyAlgorithm(), currentParam.getSymmetricAlgorithm(), createS2K(currentParam.getHashAlgorithm()), currentParam.getPassPhrase(), key[n].getKeyData().getKey());
                    }
                    ...
                    if (key[n] instanceof OpenPGPKeyData) { // if this is an OpenPGPKeyData key then try and import the key packet.
                        ...
                        currentKeyPacket = new SecretSubkeyPacket(currentParam.getCreationDate(), currentParam.getPublicKeyAlgorithm(), currentParam.getSymmetricAlgorithm(), createS2K(currentParam.getHashAlgorithm()), currentParam.getPassPhrase(), key[n].getKeyData().getKey());
                    }
                }
                ...
                if (n>0) { // sub key (sign with primary key)
                    byte [] tmp = generateSubKeyHashData(primaryKeyPacket.encodePacketBody(), currentKeyPacket.encodePacketBody());
                   
                    out.writePacket(new SignaturePacket(generateSubkeySignature(key[0].getKeyData().getKey().getPrivateKey(), primaryKeyPacket.getKeyID(), currentParam, tmp)));
                }
            }
    ...
    public String getDescription() {
    	return "OpenPGP ASCII Secret File";
    }
}
