...
public class ConfigurationWindow extends javax.swing.JDialog {
    ...
    private void applySettings() {
    	...
    	// keyring paths
    	String pubfile = StringHelper.escapeWinPath(
    			configurationData.getSetting("keymanager.openpgp.primary.pubring", 
    					"pubring.pgp"));
        String secfile = StringHelper.escapeWinPath(
        		configurationData.getSetting("keymanager.openpgp.primary.secring", 
        				"secring.pgp"));
        configurationData.setSetting("keymanager.openpgp.primary.pubring",pubfile);
        configurationData.setSetting("keymanager.openpgp.primary.secring",secfile);
        
        // key handler paths
        int n = 1;
        Map pubPathList = new HashMap();
        while (configurationData.getSetting(
        		"keymanager.openpgp.publiclist."+n+".type", "").compareTo("")!=0) {
            String filePath = StringHelper.escapeWinPath(
            		configurationData.getSetting(
            				"keymanager.openpgp.publiclist."+n+".filename", ""));
            pubPathList.put(new Integer(n), filePath);
            n++;
        }
        for (Iterator it = pubPathList.keySet().iterator(); it.hasNext();) {
        	Integer num = (Integer)it.next();
        	configurationData.setSetting("keymanager.openpgp.publiclist." + 
        			num.intValue() + ".filename",(String)pubPathList.get(num));
        }
        
        n = 1;
        Map secPathList = new HashMap();
        while (configurationData.getSetting(
        		"keymanager.openpgp.secretlist."+n+".type", "").compareTo("")!=0) {
        	String filePath = StringHelper.escapeWinPath(
        			configurationData.getSetting(
        					"keymanager.openpgp.secretlist."+n+".filename", ""));
            secPathList.put(new Integer(n), filePath);
            n++;
        }
        for (Iterator it = secPathList.keySet().iterator(); it.hasNext();) {
        	Integer num = (Integer)it.next();
        	configurationData.setSetting("keymanager.openpgp.secretlist." + 
        			num.intValue()+".filename",(String)secPathList.get(num));
        }
