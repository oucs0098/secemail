...
/**
 * <p>This test extracts a public key from a file, generates a public key 
 * encrypted session key packet using that public key and encrypts 
 * some literal data packets inside a data packet.</p>
 * <p>The resultant packet can be decoded using a ready made pgp program 
 * to test that the format is consistent with the pgp rfc.</p>
 * This class is based on the TestSymmetricallyEncryptedDataPacket test.
 */
public class TestSymmetricallyEncryptedIntegrityProtectedDataPacket extends
		TestCase {
    ...
    public boolean doTest( int pkAlgorithm, int symmetricAlgorithm )
    {
        ...
        try
        {
			...
			// Extract signature
			OpenPGPPacketInputStream sigin = new OpenPGPPacketInputStream(
					getClass().getResourceAsStream( publickeysigfile ));
			SignaturePacket sp = (SignaturePacket)sigin.readPacket();
			sigin.close();
			
			// decode public key and generate a public key object
			OpenPGPPacketInputStream tmpin = new OpenPGPPacketInputStream(
			...
			RSAPublicKeySpec publickeyspec = new RSAPublicKeySpec(
					keydata.getN(), keydata.getE());
			KeyFactory keyFactory = KeyFactory.getInstance(
					PublicKeyAlgorithmSettings.getCipherText(pkAlgorithm));
			...
			KeyGenerator k = KeyGenerator.getInstance(
					SymmetricAlgorithmSettings.getCipherText(symmetricAlgorithm), "BC");
			k.init(SymmetricAlgorithmSettings.getDefaultKeySize(symmetricAlgorithm), 
			...
			System.out.println("Creating new PubKey Encrypted Session Key Packet...");
			PublicKeyEncryptedSessionKeyPacket pkeskp = 
					new PublicKeyEncryptedSessionKeyPacket(
							publickey,sp.getKeyID(),pkAlgorithm,sk);
			
			// Generate data packet
			System.out.println("Creating new Symmetrically Encrypted Data packet...");
			SymmetricallyEncryptedIntegrityProtectedDataPacket dp = 
					new SymmetricallyEncryptedIntegrityProtectedDataPacket();
			
			System.out.println("Creating and adding first literal packet...");
			LiteralDataPacket p1 = new LiteralDataPacket(format_1, filename_1,  
					rawdata_1.getBytes());
			
			System.out.println("Creating and adding second literal packet...");
			LiteralDataPacket p2 = new LiteralDataPacket(format_2, filename_2,  
					rawdata_2.getBytes());
			...
			System.out.println("  Opening packet stream to "+outputfile+"...");
			OpenPGPPacketInputStream instream = new OpenPGPPacketInputStream(
					new FileInputStream(outputfile));
			
			System.out.println("  Reading Session key packet (and discarding)...");
			PublicKeyEncryptedSessionKeyPacket r_skp = 
					(PublicKeyEncryptedSessionKeyPacket)instream.readPacket();
			
			System.out.println("  Reading Data packet ...");
			EncryptedDataPacket r_p = (EncryptedDataPacket)instream.readPacket();
    ...
    public void testSEIPDPWithRSAAndAES128()
    {
    	assertTrue( doTest( 
    			PublicKeyAlgorithmSettings.RSA_ENCRYPTSIGN, 
    ...
    public void testSEIPDPWithRSAAndAES192()
    {
    	assertTrue( doTest( 
    			PublicKeyAlgorithmSettings.RSA_ENCRYPTSIGN, 
    ...
    public void testSEIPDPWithRSAAndAES256()
    {
    	assertTrue( doTest( 
    			PublicKeyAlgorithmSettings.RSA_ENCRYPTSIGN, 
